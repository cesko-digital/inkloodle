...
class block_edmo_about_area_two_edit_form extends block_edit_form {
    ...
    protected function specific_definition($mform) {
        ...
        $mform->setDefault('config_title', 'Build Your Skills Online, Anytime');
        $mform->setType('config_title', PARAM_RAW);

        // Content
        $mform->addElement('textarea', 'config_body', get_string('config_body', 'theme_edmo'));
        $mform->setDefault('config_body', '<p>Want to learn and earn PDUs or CEUs on your schedule — anytime, anywhere? Or, pick up a new skill quickly like, project team leadership or agile? Browse our most popular online courses.</p>');
        ...
        // Images
        $mform->addElement('filemanager', 'config_image', 'Section Image(Two Image)', null,  array('subdirs' => 0, 'maxbytes' => 10485760, 'areamaxbytes' => 10485760, 'maxfiles' => 2, 'accepted_types' => array('.png', '.jpg', '.gif') ));
        ...
        $mform->addElement('text', 'config_shape_img6', 'Shape Image 6 URL');
        $mform->setType('config_shape_img6', PARAM_TEXT);
        
        $mform->addElement('text', 'config_shape_img7', 'Shape Image 7 URL');
        $mform->setType('config_shape_img7', PARAM_TEXT);
        
        $mform->addElement('text', 'config_shape_img8', 'Shape Image 8 URL');
        $mform->setType('config_shape_img8', PARAM_TEXT);
        
        $mform->addElement('text', 'config_shape_img9', 'Shape Image 9 URL');
        $mform->setType('config_shape_img9', PARAM_TEXT);
    }
    ...
    function set_data($defaults)
    {
        ...
        file_prepare_draft_area($draftitemid, $this->block->context->id, 'block_edmo_about_area_two', 'content', 0,
            array('subdirs' => true));
        $entry->attachments = $draftitemid;
        parent::set_data($defaults);
        if ($data = parent::get_data()) {
            file_save_draft_area_files($data->config_image, $this->block->context->id, 'block_edmo_about_area_two', 'content', 0,
                array('subdirs' => true));
        }
