...
class block_edmo_banner_3 extends block_base {
    public function init() {
        $this->title = get_string('pluginname', 'block_edmo_banner_3');
    }
    ...
    public function specialization()
    {
        ...
        if (empty($this->config)) {
            ...
            $this->config->title = 'Project Management Certification Training';
    ...
    public function get_content() {
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files($this->context->id, 'block_edmo_banner_3', 'content');
        $text = '';
        $text .= '
        <!-- Start Banner Section Area -->';
        if($this->content->bg_img):
            $text .='
            <div class="banner-section '.$this->content->class.'" style="background-image:url('.edmo_block_image_process($this->content->bg_img).');">';
        else:
            $text .='
            <div class="banner-section '.$this->content->class.'">';
        endif;
        $text .='
                    ...
                    <div class="col-lg-7 col-md-12">
                        <div class="banner-image">';
                            ...
                            if($this->content->shape_img1):
                                ...
                                $text .= '
                                <div class="banner-shape4" data-speed="0.06" data-revert="true"><img src="'.edmo_block_image_process($shape_img1).'" alt="'.$this->content->title.'"></div>';
                            endif;

                            if($this->content->shape_img2):
                                $shape_img2 = $this->content->shape_img2;
                                $text .= '
                                <div class="banner-shape5" data-speed="0.06" data-revert="true"><img src="'.edmo_block_image_process($shape_img2).'" alt="'.$this->content->title.'"></div>';
                            endif;

                            if($this->content->shape_img3):
                                $shape_img3 = $this->content->shape_img3;
                                $text .= '
                                <div class="banner-shape6" data-speed="0.06" data-revert="true"><img src="'.edmo_block_image_process($shape_img3).'" alt="'.$this->content->title.'"></div>';
                            endif;

                            if($this->content->shape_img4):
                                $shape_img4 = $this->content->shape_img4;
                                $text .= '
                                <div class="banner-shape7" data-speed="0.06" data-revert="true"><img src="'.edmo_block_image_process($shape_img4).'" alt="'.$this->content->title.'"></div>';
                            endif;
                            $text .= '
                        </div>
                    </div>
