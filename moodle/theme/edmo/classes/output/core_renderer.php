...
class core_renderer extends \core_renderer {
    ...
    public function edmo_is_siteadmin() {
        if (is_siteadmin()) {
   ...
   */
    ...
    public function if_home_pages() {
        ...
        $hide_banner      = get_config('theme_edmo', 'hide_banner');
        ...
        if($hide_global_banner == 0){
            if($hide_banner){
                foreach(preg_split("/((\r?\n)|(\r\n?))/", $hide_banner) as $line){
                    ...
                    $currentUrlParts = parse_url($actual_link);
                    $currentQueryParams = [];
                    parse_str($currentUrlParts['query'], $currentQueryParams);

                    $newUrlParts = parse_url($line);
                    $newQueryParams = [];
                    parse_str($newUrlParts['query'], $newQueryParams);

                    if (isset($currentQueryParams['id']) && isset($newQueryParams['id']) && $currentQueryParams['id'] == $newQueryParams['id']) {
                        return true;
                    }
                }
            }
    ...
    public function if_hide_page_bottom_content() {
        ...
        $hide_page_bottom_content      = get_config('theme_edmo', 'hide_page_bottom_content');
     ...
     * @return string
     */
    public function edmo_if_user_enrolled() {
    ...
    public function user_menu($user = null, $withlinks = null) {
        ...
        if ($withlinks) {
            ...
            foreach ($opts->navitems as $key => $value) {
                ...
                switch ($value->itemtype) {
                    ...
                    case 'link':                        
                        $edmo_nav_items .= '<a class="dropdown-item" href="'. $value->url .'">'. $value->title .'</a>';
        ...
        return $edmo_nav_items;
    }

    /**
     * region_main_settings_menu
     */
    public function edmo_region_main_settings_menu() {
        global $OUTPUT;
